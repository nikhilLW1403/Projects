...
void CreateDILB()
{
	...
	while(i <= MAXINODE)
	{
		...
		newn->LinkCount = newn->RefferenceCount =0;
...
PINODE Get_Inode(char *name)
{
	...
	return temp; // temp mde NULL jail
}
int CreateFile(char *name, int permission)
{
	...
	if(UFDTArr[i].ptrfiletable==NULL)  //memory allocate nahi zali tr
	{
		return -4; 
	}
...
int WriteFile(int fd,char *arr,int iSize)
{
	...
	//strncpy copy n character from src to dest
...
int main()
{
	...
	while(1)
	{
		...
		if(count==1)
		{
			...
			{
				CloseAllFile();
				printf("All files are successfully\n");
				continue;
			}
		...
		else if(count==3)
		{
			...
			else if(_stricmp(command[0],"read")==0)
			{
				...
				ret= ReadFile(fd,ptr,atoi(command[2])); //atoi = convert string to integer
				if(ret==-1)
